...
const Read = () => {
  ...
  const mainElement = (
    <div className='read-route'>
      <div className={ classNames('ui message', { negative: isDeleted }, { noshow: !isDeleted })}>
          ...
          { apiData.map((user, idx) => {
            return (
              <Table.Row key={ user.id }>
                ...
                <Table.Cell>
                  <Link to="/update"><Button className="ui button primary" onClick={() => handleUpdate(user)}>Update</Button></Link>
                  <Button className="ui button negative" onClick={() => handleDelete(user.id)}>Delete</Button>
                </Table.Cell>
            </Table.Row>
            )
          })}
